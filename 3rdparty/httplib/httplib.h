...
inline bool process_client_socket(socket_t sock,
                                  ...
                                  std::function<bool(Stream &)> callback) {
    SocketStream strm(
        sock, read_timeout_sec, read_timeout_usec, write_timeout_sec, write_timeout_usec, max_timeout_msec, start_time);
...
read_content_chunked(Stream &strm, T &x, size_t payload_max_length, ContentReceiverWithProgress out) {
    ...
    thread_local case_ignore::unordered_set<std::string> prohibited_trailers = {// Message framing
                                                                                "transfer-encoding",
                                                                                "content-length",
        ...
        x, status, std::move(receiver), decompress, [&](const ContentReceiverWithProgress &out) {
            ...
            if (is_chunked_transfer_encoding(x.headers)) {
                ...
                auto len = get_header_value_u64(
                    x.headers, "Content-Length", (std::numeric_limits<size_t>::max)(), 0, is_invalid_value);
...
public:
    ...
    bool
    parse(const char *buf, size_t n, const FormDataHeader &header_callback, const ContentReceiver &content_callback) {
...
inline std::string
serialize_multipart_formdata(const UploadFormDataItems &items, const std::string &boundary, bool finish = true) {
    std::string body;
...
inline bool SocketStream::wait_readable() const {
    ...
    time_t read_timeout_usec;
    calc_actual_timeout(
        max_timeout_msec_, duration(), read_timeout_sec_, read_timeout_usec_, read_timeout_sec, read_timeout_usec);
    ...
    if (res.content_length_ > 0) {
        if (req.ranges.empty()) {
            ...
            return detail::write_content(
                strm, res.content_provider_, offset_and_length.first, offset_and_length.second, is_shutting_down);
        } else {
...
inline bool Server::process_request(Stream &strm,
    ...
    if (routed) {
        ...
        if (!res.file_content_path_.empty()) {
            ...
            res.set_content_provider(
                mm->size(), content_type, [mm](size_t offset, size_t length, DataSink &sink) -> bool {
                    sink.write(mm->data() + offset, length);
                    return true;
                });
        }
    ...
    auto ret = detail::process_server_socket(
        ...
        [&](Stream &strm, bool close_connection, bool &connection_closed) {
            return process_request(
                strm, remote_addr, remote_port, local_addr, local_port, close_connection, connection_closed, nullptr);
        });
...
inline Result ClientImpl::Post(const std::string &path,
                               ...
                               UploadProgress progress) {
    return send_with_content_provider(
        "POST", path, headers, nullptr, content_length, std::move(content_provider), nullptr, content_type, progress);
}
...
inline Result ClientImpl::Post(const std::string &path,
                               ...
                               UploadProgress progress) {
    return send_with_content_provider(
        "POST", path, headers, nullptr, 0, nullptr, std::move(content_provider), content_type, progress);
}
...
inline Result ClientImpl::Put(const std::string &path,
                              ...
                              UploadProgress progress) {
    return send_with_content_provider(
        "PUT", path, headers, nullptr, content_length, std::move(content_provider), nullptr, content_type, progress);
}
...
inline Result ClientImpl::Put(const std::string &path,
                              ...
                              UploadProgress progress) {
    return send_with_content_provider(
        "PUT", path, headers, nullptr, 0, nullptr, std::move(content_provider), content_type, progress);
}
...
inline Result ClientImpl::Patch(const std::string &path,
                                ...
                                UploadProgress progress) {
    return send_with_content_provider(
        "PATCH", path, headers, nullptr, content_length, std::move(content_provider), nullptr, content_type, progress);
}
...
inline Result ClientImpl::Patch(const std::string &path,
                                ...
                                UploadProgress progress) {
    return send_with_content_provider(
        "PATCH", path, headers, nullptr, 0, nullptr, std::move(content_provider), content_type, progress);
}
...
inline Result Client::Get(const std::string &path,
                          ...
                          DownloadProgress progress) {
    return cli_->Get(
        path, params, headers, std::move(response_handler), std::move(content_receiver), std::move(progress));
}
...
[[deprecated("Use set_path_encode instead")]] inline void Client::set_url_encode(bool on) { cli_->set_path_encode(on); }
