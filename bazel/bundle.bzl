...
def _bundle_transitive_files(ctx, transitive_func):
    ...
    srcs = [
        f
        for f in transitive_func(srcs).to_list()
...
def _bundle_repack_impl(ctx):
    dest_src_map = {}
    for e in ctx.attr.srcs:
        for dest, src in e[BundleFilesInfo].dest_src_map.items():
            dest = paths.join(ctx.attr.prefix, dest)
            if dest in dest_src_map:
                prev = dest_src_map[dest]
                if src == prev:
                    continue
                else:
                    fail("key [{}] duplicated in bundle_files, previous [{}] current [{}]".format(dest, prev, src))
            dest_src_map[dest] = src
    return [
        BundleFilesInfo(dest_src_map = dest_src_map),
    ]
...
bundle_repack_impl = rule(
    implementation = _bundle_repack_impl,
    attrs = {
        "srcs": attr.label_list(),
        "prefix": attr.string(default = "./"),
        "execution_requirements": attr.string_dict(),
