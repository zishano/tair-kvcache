load("//bazel:bundle.bzl", "BundleFilesInfo", "bundle_files", "bundle_tar", "mapping_files")
load("//bazel:defs.bzl", "if_arm")

ATTR_ASPECTS = ["deps", "srcs", "libs"]

BundleStripFilesInfo = provider(fields = ["dest_src_map"])
BundleDebugFilesInfo = provider(fields = ["dest_src_map"])
KeepSymbolInfo = provider(fields = ["target"])  # keep debug symbols in seperated file
OriginalSymbolInfo = provider(fields = ["target"])  # keep debug symbols within original file
...
def _keep_symbol_collector_aspect_impl(target, ctx):
    ...
    if hasattr(attr, "tags"):
        if "fuse-keep-symbol" in attr.tags:
            ...
            if BundleFilesInfo in target:
                dest_src_map = target[BundleFilesInfo].dest_src_map
                collected.extend([src for dest, src in dest_src_map.items() if dest.endswith(".so")])
    ...
    for field in ATTR_ASPECTS:
        _travel_sub_attr(field)
...
def _original_symbol_collector_aspect_impl(target, ctx):
    # This function is executed for each dependency the aspect visits.
    collected = []
    attr = ctx.rule.attr
    if hasattr(attr, "tags"):
        if "fuse-original-symbol" in attr.tags:
            collected.extend(target.files.to_list())
    ...
    def _travel_sub_attr(name):
        ...
        if type(sub_attr) == "list":
            for e in sub_attr:
                if OriginalSymbolInfo in e:
                    collected.extend(e[OriginalSymbolInfo].target)

    for field in ATTR_ASPECTS:
        _travel_sub_attr(field)
    return [OriginalSymbolInfo(target = collected)]

original_symbol_collector_aspect = aspect(
    implementation = _original_symbol_collector_aspect_impl,
    attr_aspects = ATTR_ASPECTS,
)

def modify_bundle_impl(ctx):
    keep_symbols_set = [f for f in ctx.files.keep_symbols]
    original_symbols_set = []
    original_src_map = {}
    final_src_map = {}
    final_dbg_map = {}
    for e in ctx.attr.deps:
        keep_symbols_set.extend(e[KeepSymbolInfo].target)
        original_symbols_set.extend(e[OriginalSymbolInfo].target)
    for e in ctx.attr.deps:
        ...
        for key in dest_src_map:
            ...
            if obj in keep_symbols_set:
                ...
                tmp_file = "/tmp/" + "strip{}.{}".format(hash(key), key.split("/")[-1])
                cmd_bash = """#!/bin/bash
set -e
cp {raw_path} {tmp_file}
chmod u+w {tmp_file}
gdb-add-index {tmp_file}
objcopy --only-keep-debug {tmp_file} {debug_path}
strip -g {tmp_file} -o {strip_path}
objcopy --add-gnu-debuglink={debug_path} {strip_path}
rm {tmp_file}
""".format(raw_path = raw_path, tmp_file = tmp_file, strip_path = strip_path, debug_path = debug_path)
            ...
            elif obj in original_symbols_set:
...
modify_bundle = rule(
    attrs = {
        "deps": attr.label_list(
            allow_files = True,
            mandatory = True,
            aspects = [keep_symbol_collector_aspect, original_symbol_collector_aspect],
        ),
...
if [[ \"$(uname -m)\" == \"aarch64\" ]]; then
    oss_download_prefix=$(echo {oss_prefix} | sed 's|oss://alibaba-aios-debuginfo|https://alibaba-aios-debuginfo.oss-cn-hangzhou.aliyuncs.com|')
    upload_filename=\"{pkg_prefix}_`date '+%Y%m%d%H%M%S'`_`md5sum {package_tar_path}|cut -d ' ' -f 1`.tar.gz\"
    ossutil cp {package_tar_path} {oss_prefix}/$upload_filename
    echo $oss_download_prefix/$upload_filename > {upload_result_path}
else
    python2 /usr/local/bin/osscmd multiupload {package_tar_path} {oss_prefix}/{pkg_prefix}_`date '+%Y%m%d%H%M%S'`_`md5sum {package_tar_path}|cut -d ' ' -f 1`.tar.gz > {upload_result_path}
fi
    """.format(
        package_tar_path = package_tar_file.path,
...
"""
    ...
    cmd_bash = """
if [[ \"$(uname -m)\" == \"aarch64\" ]]; then
    echo `cat {upload_result_path}` > {debug_symbol_url_file}
else
    echo `tail -n 3 {upload_result_path} | grep "Object URL" | cut -d ' ' -f 4| sed 's/http:/https:/g'| sed 's#%2F#/#g' ` > {debug_symbol_url_file}
fi
    """.format(
        upload_result_path = upload_result_file.path,
...
_handle_debug_bundle = rule(
    attrs = {
        ...
        "oss_prefix": attr.string(
            default = "oss://alibaba-aios-debuginfo/aios_dev/default",
        ),
...
def debug_info_version_file(name, debug_version_file):
    native.genrule(
        ...
        cmd = "cat bazel-out/stable-status.txt bazel-out/volatile-status.txt | grep -E \"GIT|USER_NAME\" > $(OUTS)",
        stamp = 1,
    )
    bundle_files(
...
def fuse_debug_bundle_files(
        name,
        deps,
        compress = True,
        keep_symbols = [],
        oss_prefix = if_arm("oss://alibaba-aios-debuginfo/aios_dev/default", "oss://alibaba-aios-debuginfo/aios_dev/default"),
        pkg_prefix = "debug_info",
        **kwargs):
