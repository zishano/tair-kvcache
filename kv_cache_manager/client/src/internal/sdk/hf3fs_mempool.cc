...
Hf3fsMempool::~Hf3fsMempool() {
    buffer_ = nullptr;
    total_size_ = 0;
}

bool Hf3fsMempool::Init() {
    if (buffer_ == nullptr || total_size_ <= 0) {
        KVCM_LOG_WARN("3fs mempool init failed, buffer or total size is invalid, buffer: %p, total size: %zu",
                      buffer_,
                      total_size_);
        return false;
    }

    {
        ...
        KVCM_LOG_DEBUG("init free size: %zu, addr: %p", total_size_, buffer_);
    }
    return true;
}

void *Hf3fsMempool::Alloc(size_t size) {
    if (size == 0) {
        KVCM_LOG_WARN("alloc failed, alloc size is 0");
        return nullptr;
    }
    ...
    if (block_addr == nullptr || block_size == 0) {
        ...
        KVCM_LOG_WARN("alloc failed, no free block, alloc size: %zu, mempool: [%s]", size, status.c_str());
        return nullptr;
    }
    ...
    KVCM_LOG_DEBUG("alloc size: %zu, addr: %p", size, block_addr);
    return static_cast<void *>(block_addr);
}

void Hf3fsMempool::Free(void *ptr) {
    if (!ptr) {
        return;
    }

    KVCM_LOG_DEBUG("free addr: %p", ptr);
    ...
    size_t block_size = 0;
    {
        ...
        if (auto alloc_it = allocated_blocks_.find(block_start); alloc_it != allocated_blocks_.end()) {
            ...
            KVCM_LOG_WARN("free memory but pointer is not allocated, ptr: %p", block_start);
            return;
        }
    }
...
size_t Hf3fsMempool::AllocatedSize() const {
...
size_t Hf3fsMempool::FreeSize() const {
...
int Hf3fsMempool::AllocatedBlockCount() const {
    std::shared_lock<std::shared_mutex> lock(allocated_blocks_mutex_);
    return static_cast<int>(allocated_blocks_.size());
}

int Hf3fsMempool::FreeBlockCount() const {
    std::shared_lock<std::shared_mutex> lock(free_blocks_mutex_);
    return static_cast<int>(free_blocks_.size());
}

std::string Hf3fsMempool::ToString() const {
...
void Hf3fsMempool::PrintStatus() const {
    printf("\n---------- Memory Pool Status ----------\n");
