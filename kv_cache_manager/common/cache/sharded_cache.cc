...
#include "kv_cache_manager/common/cache/math.h"
...
uint32_t DetermineSeed(int32_t hash_seed_option) {
    ...
    uint32_t val = Lower32of64(now) & kSeedMask;
...
size_t ShardedCacheBase::GetPerShardCapacity() const { return ComputePerShardCapacity(GetCapacity()); }

uint64_t ShardedCacheBase::NewId() { return last_id_.fetch_add(1, std::memory_order_relaxed); }
...
ErrorCode ShardedCacheBase::GetSecondaryCacheCapacity(size_t &size) const {
    size = 0;
    return EC_OK;
}

ErrorCode ShardedCacheBase::GetSecondaryCachePinnedUsage(size_t &size) const {
    size = 0;
    return EC_OK;
}
...
size_t ShardedCacheBase::GetUsage(Handle *handle) const { return GetCharge(handle); }
...
int ShardedCacheBase::GetNumShardBits() const { return BitsSetToOne(shard_mask_); }
