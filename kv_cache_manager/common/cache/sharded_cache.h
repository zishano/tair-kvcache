...
#include "kv_cache_manager/common/cache/port_posix.h"
...
public:
    ...
    static inline uint32_t HashPieceForSharding(HashCref hash) { return Lower32of64(hash); }
...
public:
    ...
    ErrorCode GetSecondaryCacheCapacity(size_t &size) const override;
    ErrorCode GetSecondaryCachePinnedUsage(size_t &size) const override;
...
public:
    ...
    explicit ShardedCache(const ShardedCacheOptions &opts)
        ...
        , shards_(static_cast<CacheShard *>(port::cacheline_aligned_alloc(sizeof(CacheShard) * GetNumShards())))
    ...
    CacheShard &GetShard(HashCref hash) { return shards_[CacheShard::HashPieceForSharding(hash) & shard_mask_]; }
    ...
    void SetStrictCapacityLimit(bool s_c_l) override {
        ...
        ForEachShard([s_c_l](CacheShard *cs) { cs->SetStrictCapacityLimit(s_c_l); });
    }
    ...
    Handle *CreateStandalone(const std::string_view &key,
        ...
        assert(helper);
        HashVal hash = CacheShard::ComputeHash(key, hash_seed_);
        HandleImpl *result = GetShard(hash).CreateStandalone(key, hash, obj, helper, charge, allow_uncharged);
    ...
    Handle *Lookup(const std::string_view &key,
        ...
        HashVal hash = CacheShard::ComputeHash(key, hash_seed_);
        HandleImpl *result = GetShard(hash).Lookup(key, hash, helper, create_context, priority, stats);
        return static_cast<Handle *>(result);
    }
    ...
    size_t GetUsage() const override { return SumOverShards2(&CacheShard::GetUsage); }
    size_t GetPinnedUsage() const override { return SumOverShards2(&CacheShard::GetPinnedUsage); }
    size_t GetOccupancyCount() const override { return SumOverShards2(&CacheShard::GetOccupancyCount); }
    size_t GetTableAddressCount() const override { return SumOverShards2(&CacheShard::GetTableAddressCount); }
...
protected:
    ...
    inline void ForEachShard(const std::function<void(const CacheShard *)> &fn) const {
        uint32_t num_shards = GetNumShards();
        for (uint32_t i = 0; i < num_shards; i++) {
            fn(shards_ + i);
        }
    ...
    inline size_t SumOverShards(const std::function<size_t(CacheShard &)> &fn) const {
        uint32_t num_shards = GetNumShards();
        size_t result = 0;
        for (uint32_t i = 0; i < num_shards; i++) {
            result += fn(shards_[i]);
    ...
    void AppendPrintableOptions(std::string &str) const override { shards_[0].AppendPrintableOptions(str); }
...
int GetDefaultCacheShardBits(size_t capacity, size_t min_shard_size = 512U * 1024U);
