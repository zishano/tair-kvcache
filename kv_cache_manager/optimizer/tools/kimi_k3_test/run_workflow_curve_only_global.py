#!/usr/bin/env python3
"""Curve-only Kimi-K3 capacity workflow with support for global hash_id_scope.

This is a modified version of run_workflow_curve_only.py that supports:
1. Global hash_id_scope (in addition to local)
2. Multiple block sizes within the same trace

Key differences from the original:
- Removed strict validation requiring hash_id_scope == "local"
- Removed strict validation requiring all sessions have the same block_size
- For global scope, hash_ids are used directly without globalization
- For multiple block sizes, each session's block_size is validated independently
"""
...
def flatten_agentx_requests(args, instance_id: str) -> tuple[list[dict], dict]:
    ...
    This version supports both 'local' and 'global' hash_id_scope:
    - local: hash_ids are session-specific, need globalization via (session_index << 32) | local_id
    - global: hash_ids are already globally unique, use them directly
    """
    result, sessions, filtered = [], [], Counter()
    total_seen = parent_checks = subagent_rows = 0
    hash_scope_stats = Counter()
    block_size_stats = Counter()
    ...
    with args.trace.open() as stream:
        for session_index, line in enumerate(stream):
            ...
            # Collect statistics instead of strict validation
            hash_scope = session.get("hash_id_scope", "unknown")
            block_size = session.get("block_size", 0)
            hash_scope_stats[hash_scope] += 1
            block_size_stats[block_size] += 1

            # Validate that block_size divides checkpoint_tokens evenly
            if block_size <= 0 or args.checkpoint_tokens % block_size:
                raise ValueError(
                    f"Session {session.get('id')} block_size={block_size} does not divide "
                    f"checkpoint_tokens={args.checkpoint_tokens} evenly"
                )
            ...
            for path, req in kept:
                ...
                expected_blocks = req["in"] // block_size
                if len(local_keys) != expected_blocks:
                    raise ValueError(
                        f"AgentX length mismatch in {session['id']} {path}: "
                        f"got {len(local_keys)} hash_ids but expected {expected_blocks} "
                        f"(input_len={req['in']} / block_size={block_size})"
                    )

                # Validate prefix-chain structure
                previous = None
                ...
                # For global scope, use hash_ids directly; for local scope, globalize them
                if hash_scope == "global":
                    keys = local_keys  # Already globally unique
    ...
    # Determine hash globalization strategy based on observed scopes
    if len(hash_scope_stats) == 1 and "global" in hash_scope_stats:
        hash_globalization = "none (source already uses global hash namespace)"
    elif len(hash_scope_stats) == 1 and "local" in hash_scope_stats:
        hash_globalization = "(source_session_line_1based << 32) | local_prefix_hash_id"
    else:
        hash_globalization = f"mixed: {dict(hash_scope_stats)} - global kept as-is, local globalized"
    ...
    return result, {
        ...
        "hash_globalization": hash_globalization,
        "hash_scope_distribution": dict(hash_scope_stats),
        "block_size_distribution": dict(block_size_stats),
