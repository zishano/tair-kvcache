...
WORKFLOW="${K3_WORKFLOW:-$SCRIPT_DIR/run_workflow_curve_only_global.py}"
...
TRACE_ORIGINAL="${K3_TRACE:-$SCRIPT_DIR/trace_0918/ed383f7f-ce5.json}"
TRACE="${TRACE_ORIGINAL%.json}_fixed.json"
OUTPUT="${K3_RUN:-$SCRIPT_DIR/output/capacity_curve_0918_1}"
...
for path in "$WORKFLOW" "$MODEL" "$TRACE_ORIGINAL"; do
  if [[ ! -f "$path" ]]; then
...
# === Auto-fix subagent timestamps if needed ===
if [[ ! -f "$TRACE" || "$TRACE_ORIGINAL" -nt "$TRACE" ]]; then
  printf '[INFO] Fixing subagent timestamps: %s -> %s\n' "$TRACE_ORIGINAL" "$TRACE"
  "$PYTHON" "$SCRIPT_DIR/fix_subagent_timestamps.py" "$TRACE_ORIGINAL" "$TRACE" || {
    printf '[ERROR] Failed to fix timestamps\n' >&2
    exit 1
  }
else
  printf '[INFO] Using existing fixed trace: %s\n' "$TRACE"
fi
